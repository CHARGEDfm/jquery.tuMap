 ...
 *
 * Modified by the engineering team at @CHARGEDfm (@HenrikR, @evride)
 * http://charged.fm
 *
...
(function(d) {
	...
	var e = {
		...
		Initialize: function(i) {
			...
			if (jQuery().hammer) {
				...
				k.on("tap", function(o) {
					...
					e.AnimateZoom(e.ZoomLevel, e.ZoomLevel + e.CalcZoomInterval(e.ZoomLevel, 5));
				}).on("pinch", function(o) {
					var p = e.Zoom * o.gesture.scale;
					$('.log').text(o.gesture.scale + " - " + p);
					if (p < 1) {
						p = 1
					} else {
						if (p > e.Resolutions[e.Resolutions.length-1]/100) {
							p = e.Resolutions[e.Resolutions.length-1]/100;
						}
					}
					e.SetZoom(p, true);
					if(e.ReloadTimeout){
					...
					e.ReloadTimeout = setTimeout(function(){
							e.SetZoom(p);
						}, 400);
				}).on("drag", function(o) {
			...
			e.Wrapper.bind("dblclick", function(evt){e.AnimateZoom(e.ZoomLevel, e.ZoomLevel + e.CalcZoomInterval(e.ZoomLevel, 3));});
		...
		GenerateTiles: function() {
				...
				var zoomStep = 0;
				for(var i in e.Resolutions){
					var currDiff = Math.max(e.Resolutions[i]/100, e.ZoomLevel) - Math.min(e.Resolutions[i]/100, e.ZoomLevel);
					if(minDiff == -1 || currDiff <= minDiff){
						minDiff = currDiff;
						zoomStep = e.Resolutions[i]/100;
					}
				}
				if(zoomStep == e.CurrentZoomStep){
					return;
				}
				e.CurrentZoomStep = zoomStep;
				
				var g = 1;
				var j = (e.BaseTileSize * zoomStep);
				if (zoomStep <= 5) {
					g = 1;
					e.TileSize = j
				} else {
				...
				e.ImagesToLoad = g * g;
				if(this.Cache[zoomStep] == undefined){
					this.Cache[zoomStep] = [];
					for (var h = 0; h < g; h++) {
						for (var k = 0; k < g; k++) {
							var i;
							i = e.GetTile(new b.Map.Point(h, k), zoomStep, document);
							i.BaseSize = e.TileSize;
							i.ZoomLevel = zoomStep;
							i.blockY = k;
							i.blockX = h;
							$(i).bind('load', function(evt){
								if(i.ZoomLevel == e.CurrentZoomStep){
									e.ImagesLoaded++; 
								}
								if(e.ImagesLoaded == e.ImagesToLoad){
									
									for (var h = 0; h < e.Tiles.length; h++) {
										if(e.Tiles[h].context.ZoomLevel != e.CurrentZoomStep){
											console.log('remove');
											e.Tiles[h].remove();
										}
									}
							...
							this.Cache["h" + h + "k" + k + "z"+ zoomStep] = i;
					...
					for (var h = 0; h < g; h++) {
						for (var k = 0; k < g; k++) {
							i = this.Cache["h" + h + "k" + k + "z"+ zoomStep];
							d(i).attr("style", "position:absolute;top:" + (k * e.TileSize) + "px;left:" + (h * e.TileSize) + "px;z-index:0;width:" + e.TileSize + "px;height:" + e.TileSize + "px;").appendTo(e.Wrapper).disableSelection();
							this.Tiles.push(d(i));
		...
		SetZoom: function(h, animating) {
			...
			for(var i in this.Tiles){
				var tSize = e.ZoomLevel / this.Tiles[i].context.ZoomLevel * this.Tiles[i].context.BaseSize;
				var tX = this.Tiles[i].context.blockX * tSize;
				var tY = this.Tiles[i].context.blockY * tSize;
				
				this.Tiles[i].css({'width':tSize+"px", 'height':tSize+"px", 'left':tX+"px", 'top':tY+"px"});
			}
			e.SetCenter(new b.Map.Point(e.Center.x * e.ZoomLevel / g, e.Center.y * e.ZoomLevel / g));
			$(e.Canvas).css({'width':e.Side+"px", 'height':e.Side+"px"});
		},
		...
		CalcZoomInterval:function(num, steps){
			var max = e.Resolutions[e.Resolutions.length-1]/100;
			return num * num / (max * 2.5) * (steps>=1?steps:1); 
		},
		ZoomIn: function() {
			
			var nZoom = e.ZoomLevel + e.CalcZoomInterval(e.ZoomLevel);
			if (nZoom > e.Resolutions[e.Resolutions.length-1]/100) {
				nZoom = e.Resolutions[e.Resolutions.length-1]/100;
				return false
			}
			e.CheckMaxZoom();
			e.AnimateZoom(e.ZoomLevel, nZoom);
			//e.SetZoom(e.ZoomLevel);
			return true
		},
		ReloadTimeout: null,
		
		AnimateZoom:function(oldZoom, newZoom){
			if(e.ReloadInterval == undefined){
				clearTimeout(e.ReloadTimeout);
			}
			$({'height':oldZoom}).animate({'height':newZoom}, {'easing':'linear','duration':200, 
				'step':function(val, tween){e.SetZoom(val, true)}, 'complete':function(){
					if(e.ReloadTimeout){
						clearTimeout(e.ReloadTimeout);
					}
					e.ReloadTimeout = setTimeout(function(){
